...
router.get('/Banana', async(req,res) => {
        await Cerebro.find({flavor: 'Banana'}).count().exec((error,sabores) => {
            if(!error)
            {
                console.log(sabores);
...
router.get('/Kiwwi', async(req,res) => {
    await Cerebro.find({flavor: 'Kiwwi'}).count().exec((error,sabores) => {
        if(!error)
        {
            console.log(sabores);
            res.status(200).json(sabores);
...
router.get('/Naranja', async(req,res) => {
    await Cerebro.find({flavor: 'Naranja'}).count().exec((error,sabores) => {
        if(!error)
        {
            console.log(sabores);
            res.status(200).json(sabores);
